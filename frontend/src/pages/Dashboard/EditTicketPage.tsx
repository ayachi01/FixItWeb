...
export default function EditTicketPage() {
  ...
  useEffect(() => {
    ...
    const fetchTicket = async () => {
      try {
        ...
        setLocation(ticket.location.id);
        setExistingImages(ticket.images || []);
      } catch (err: any) {
        toast.error("❌ Failed to load ticket data");
        navigate(-1); // go back if fails
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const combined = [...images, ...newFiles].slice(
      0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // ✅ Send existing images as separate fields
    ...
    try {
      ...
      navigate(`/dashboard/tickets/${id}`); // back to ticket detail
    } catch (err: any) {
      console.error(err);
