import 'dart:io'; // ✅ Added to support File
...
class CreateReportState extends State<CreateReport> {
  ...
  // 🧠 Prefill fields from LLM data (suggestions only)
  void _prefillFromLLM(Map<String, dynamic>? data) {
    if (data == null) return;
    debugPrint("🧠 Prefilling from LLM: $data");

    final intent = (data['intent'] ?? '').toString().trim();
    final item = (data['item'] ?? '').toString().trim();
    String combined = [
      if (intent.isNotEmpty && intent != 'null') intent,
      if (item.isNotEmpty && item != 'null') item,
    ].join(' ').trim();

    if (combined.isNotEmpty) {
      combined = combined[0].toUpperCase() + combined.substring(1);
    }
    incidentType.text = combined;

    if (data['notes'] != null &&
        data['notes'] != 'null' &&
        (data['notes'] as String).trim().isNotEmpty) {
      description.text = data['notes'].toString().trim();
    }
  }
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                // Image Picker (shows preloaded image too)
                Container(
